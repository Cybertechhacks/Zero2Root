...
!!! info "Key Principle"
    Certifications open doors — skills keep you employed. The best certification is the one that forces you to practice hands-on, not just memorize. An OSCP with genuine lab hours is worth more than three theory-only certifications.
...
**What it covers:** Broad security fundamentals — network security, identity management, cryptography, threats, compliance.
...
**What it covers:** Entry-level pentesting — host/network discovery, enumeration, basic web attacks, Metasploit fundamentals.

**Exam format:** Hands-on labs — you get access to a network and must answer questions by actually compromising systems.
...
**What it covers:** Practical pentesting — network enumeration, web attacks, buffer overflows, privilege escalation, Active Directory.

**Exam format:** 24-hour hands-on exam — you must compromise a set of machines in a private network and submit a professional report within the following 24 hours.
...
**Difficulty:** Hard. Fail rate is high on first attempt. Requires genuine skill — you can't memorize your way through 24 hours of hands-on exploitation.
...
**What it covers:** Practical ethical hacking — OSINT, network scanning, exploitation, Active Directory attacks, web attacks, post-exploitation, reporting.
...
**What it covers:** Web application penetration testing — OWASP Top 10, Burp Suite, manual testing methodology.
...
**Value:** Highly credible, vendor-neutral. SANS training is excellent. Expensive — employer sponsorship typical.
...
**What it covers:** Red team operations — C2 (Cobalt Strike), OPSEC, evasion, initial access techniques, Active Directory attacks.
...
**What it covers:** Advanced Active Directory attacks — trust attacks, constrained/unconstrained delegation, ADCS, cross-forest attacks.

**Exam format:** Hands-on lab — compromise a multi-domain, multi-forest AD environment.
...
**What it covers:** Advanced web application attacks — authentication bypass chains, deserialization, SSRF chains.
...
**What it covers:** Web application security — all major vulnerability classes using Burp Suite.

**Exam format:** 4-hour proctored exam — two web apps to compromise.
...
### OSCP → OSEP → CRTO Progression

This combination covers the major pentesting domains comprehensively:

```
1. OSCP (or PNPT)     → Foundation: methodology, network, web, basic AD
2. CRTO               → Red team: C2, OPSEC, AD attacks in depth
3. OSEP or CRTE       → Advanced evasion or advanced AD/multi-forest
4. BSCP or OSWE       → Web specialization (if web is your focus)
```
...
```
CompTIA Security+ → CEH / CEH Master → PNPT → OSCP → CRTO
```
...
| CISSP | (ISC)² | Security management (requires 5yr experience) |
